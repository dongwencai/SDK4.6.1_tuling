...
typedef struct duer_ds_rec_compress_info_s {
    duer_u32_t min_time;
    duer_u32_t max_time;
    float avg_time;
    duer_u32_t frames_counter;
    duer_size_t length;
} duer_ds_rec_compress_info_t;

/**
 * reset compress infomation
 */
duer_status_t duer_ds_rec_compress_info_reset(
        duer_ds_rec_compress_info_t *module, const char *mod_name);

/**
 * update compress infomation,set start/end = duer_timestamp() before/after compress functions
 */
duer_status_t duer_ds_rec_compress_info_update(const duer_u32_t start, const duer_u32_t end,
        const duer_size_t frame_length, duer_ds_rec_compress_info_t *module, const char *mod_name);

/**
 * pack compress infomation into a baidu_json
 */
duer_status_t duer_ds_rec_compress_info_pack(
        baidu_json *message, const duer_ds_rec_compress_info_t *module, const char *mod_name);

/**
 * common function to report the log of recorder module
 */
duer_status_t duer_ds_log_rec(duer_ds_log_rec_code_t log_code, const baidu_json *log_message);
...
extern duer_ds_rec_compress_info_t g_rec_compress_info;
extern duer_ds_rec_compress_info_t g_speex_compress_info;
