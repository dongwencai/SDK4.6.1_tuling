...
TaskHandle_t mp3_codec_task_handle = NULL;
...
static void mp3_codec_buffer_function_init(mp3_codec_media_handle_t *handle)
{
    ...
    handle->flush              = mp3_codec_flush;/////////////maggie
}
...
{
    xTaskCreate(mp3_codec_task_main, MP3_CODEC_TASK_NAME, MP3_CODEC_TASK_STACKSIZE / sizeof(StackType_t), NULL, MP3_CODEC_TASK_PRIO, &mp3_codec_task_handle);    
}
...
//	printf("mp3_codec_i2s_tx_isr_callback() +,  i2s_port=%d, event=%d \r\n",  i2s_port, event);//maggie 
...
//maggie add end
    ...
    switch (event) {
        ...
        case HAL_I2S_EVENT_DATA_REQUEST:
            ...
            for (loop_idx = 0; loop_idx < 1; loop_idx++) {
...
//                printf("[MP3 Codec] isr stream_out_pcm_buffer_data_tone_amount=%d\r\n", stream_out_pcm_buffer_data_tone_amount);//maggie
                if(stream_out_pcm_buffer_data_tone_amount > 0) {
                    ...
                    if (internal_handle->is_set_eof_called == 0) {//maggie add
            ...
            if (ring_buffer_get_space_byte_count(&internal_handle->stream_out_pcm_buff) >= internal_handle->decode_pcm_buffer_size) {
				//printf("ring buffer space count=%d, decode_pcm_buffer_size=%d \r\n", ring_buffer_get_space_byte_count(&internal_handle->stream_out_pcm_buff) , internal_handle->decode_pcm_buffer_size);//maggie
                mp3_codec_event_send_from_isr(MP3_CODEC_QUEUE_EVENT_DECODE, NULL);
            }
...
//maggie add end
    ...
    //int32_t share_data_amount = mp3_codec_get_share_buffer_data_count(handle);//maggie
...
static mp3_codec_function_return_state_t mp3_codec_play_internal(mp3_codec_media_handle_t *handle)
{
    ...
    //mp3_codec_request_data_to_share_buffer(handle); // since we decoded some data, thus we request again to fill share buffer.//maggie
...
mp3_codec_function_return_state_t mp3_codec_close(mp3_codec_media_handle_t *handle)
{
    ...
    if (mp3_codec_task_handle != NULL) {

        vTaskDelete(mp3_codec_task_handle);
		mp3_codec_task_handle = NULL;
    }
