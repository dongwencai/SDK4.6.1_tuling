...
#define LOGI(fmt, arg...)          LOG_I(common, "[AUD_FMR]"fmt, ##arg);
#define LOGW(fmt, arg...)          LOG_W(common, "[AUD_FMR]"fmt, ##arg);
#define LOGE(fmt, arg...)          LOG_E(common, "[AUD_FMR]"fmt, ##arg);
...
int32_t audio_file_manager_create_file_db(TCHAR * folder_name)
{
    ...
    while ((fres = f_readdir(&dir, &fno)) == FR_OK) {
        ...
        LOGI("scanned file name3: %s, len: %u", rec.file_name, strlen(rec.file_name));
...
FRESULT audio_file_manager_close(FIL *fp)
{
    FRESULT fs_res = FR_OK;
	fs_res = f_close(fp);
    return fs_res;
...
#ifndef MTK_MP3_ON_FLASH_ENABLE
static FRESULT audio_file_manager_set_breakpoint_file_by_folder(local_mp3_breakpoint_data_t *bp_data)
{
    FRESULT res = FR_OK;
    int32_t file_ret;
    //FIL fp;
   ...
   // printf("update bp file\r\n");
    ...
    if (res == FR_OK) {
        UINT write_p;
        //uint8_t *p;
        res = f_write(&temp_fp, bp_data, sizeof(local_mp3_breakpoint_data_t), &write_p);
        //p = (uint8_t *)bp_data;

        //LOGI(AudPly, "write_p %d, size %d, DUmp %x, %x, %x, %x, %x, %x, %x, %x", write_p, sizeof(local_mp3_breakpoint_data_t),
...
static FRESULT audio_file_manager_get_breakpoint_file_by_folder(local_mp3_breakpoint_data_t *bp_data)
{
    FRESULT res = FR_OK;
    int32_t file_ret;
    //FIL fp;
    ...
    if (res == FR_OK) {
        ...
        LOGI("get breakpoint file len %d", read_len);
        ...
        if (res == FR_OK) {
            UINT write_p;
            f_write(&temp_fp,(void *)bp_data, sizeof(local_mp3_breakpoint_data_t), &write_p);
            f_close(&temp_fp);
        }
...
{
    FRESULT res = FR_OK;
    local_mp3_breakpoint_data_t *bp_data = (local_mp3_breakpoint_data_t *)data;
#ifndef MTK_MP3_ON_FLASH_ENABLE
    //FIL fp;
    UINT read_len;
